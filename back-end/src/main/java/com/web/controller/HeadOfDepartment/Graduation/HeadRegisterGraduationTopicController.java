...
public class HeadRegisterGraduationTopicController {
    ...
    @Autowired
    public HeadRegisterGraduationTopicController(TokenUtils tokenUtils){
    ...
    public ResponseEntity<?> lecturerRegisterTopic(@RequestParam("subjectName") String name,
        ...
        try {
            ...
            String token = tokenUtils.extractToken(authorizationHeader);
            Person personCurrent = CheckRole.getRoleCurrent2(token, userUtils, personRepository);
            if (personCurrent.getAuthorities().getName().equals("ROLE_HEAD") || personCurrent.getAuthorities().getName().equals("ROLE_HEAD") ) {
