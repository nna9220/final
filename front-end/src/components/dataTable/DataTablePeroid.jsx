...
function DataTablePeroid() {
  ...
  const loadData = () => {
    ...
    if (tokenSt) {
      axiosInstance.get('/admin/time/registration-periods', {
        headers: {
  ...
  const handleAddPeroid = async () => {
    ...
    if (tokenSt) {
      ...
      try {
        const response = await axiosInstance.post('/admin/time/create', formData, {
          headers: {
            'Authorization': `Bearer ${tokenSt}`,
  ...
  const handleDetailPeroid = (id) => {
    ...
    axiosInstance.get(`/admin/time/details/${id}`, {
      headers: {
